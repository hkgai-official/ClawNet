// src/main/features/agents/commands/file-move.ts
//
// 1:1 port of macOS FileCommandHandler.swift:309-355.
...
export interface FileMoveHandlerDeps {
...
export function makeFileMoveHandler(deps: FileMoveHandlerDeps): NodeCommandHandler {
  return async (ctx: NodeCommandContext) => {
    ...
    try { await rename(source, destination); } catch (err) {
