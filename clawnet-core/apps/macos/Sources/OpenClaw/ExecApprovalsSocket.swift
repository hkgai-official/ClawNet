...
enum ExecApprovalsPromptPresenter {
    ...
    static func prompt(_ request: ExecApprovalPromptRequest) -> ExecApprovalDecision {
        ...
        let hostLabel = Self.hostDescription(request.host)
        let agentLabel = request.agentId?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if !agentLabel.isEmpty {
            alert.messageText = "Allow command from \"\(agentLabel)\"?"
        } else {
            alert.messageText = "Allow this command?"
        }
        alert.informativeText = hostLabel
    ...
    private static func hostDescription(_ host: String?) -> String {
        let trimmed = host?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        switch trimmed {
        case "node":
            return "A remote agent is requesting to run this command on this Mac (via node)."
        case "gateway":
            return "The gateway is requesting to run this command on this machine."
        case "":
            return "Review the command details before allowing."
        default:
            return "Host \"\(trimmed)\" is requesting to run this command."
        }
    }
