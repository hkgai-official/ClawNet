...
struct GeneralSettings: View {
    ...
    @State private var tokenLoaded: Bool = false

    private var gatewayTokenCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                if self.showToken {
                    TextField("Token", text: self.$gatewayToken)
                        .textFieldStyle(.roundedBorder)
                        .font(.body.monospaced())
                } else {
                    SecureField("Token", text: self.$gatewayToken)
                        .textFieldStyle(.roundedBorder)
                }
                Button(self.showToken ? "Hide" : "Show") {
                ...
                Button {
                    ...
                    NSPasteboard.general.setString(self.gatewayToken, forType: .string)
                } label: {
                ...
                Button("Save") {
                    self.saveGatewayToken()
                }
                .buttonStyle(.borderedProminent)
                .disabled(self.gatewayToken.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
            }

            Text("Used for authenticating node connections to the gateway.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
        .padding(12)
        ...
        .cornerRadius(10)
        .onAppear { self.loadGatewayToken() }
    }

    private func loadGatewayToken() {
        guard !self.tokenLoaded else { return }
        self.tokenLoaded = true
        let root = OpenClawConfigFile.loadDict()
        let isRemote = self.state.connectionMode == .remote
        if isRemote {
    ...
    private func saveGatewayToken() {
        let trimmed = self.gatewayToken.trimmingCharacters(in: .whitespacesAndNewlines)
