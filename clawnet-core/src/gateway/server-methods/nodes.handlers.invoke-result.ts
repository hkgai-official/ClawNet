...
export const handleNodeInvokeResult: GatewayRequestHandler = async ({
  ...
  const callerRole = client?.connect?.role;
  const callerNodeId = client?.connect?.device?.id ?? client?.connect?.client?.id;
  if (callerRole === "node") {
    // Direct node: must match its own nodeId.
    if (callerNodeId && callerNodeId !== p.nodeId) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "nodeId mismatch"));
      return;
    }
  } else if (callerRole === "operator") {
    // Operator proxy: verify the node is a proxy owned by this connection.
    const node = context.nodeRegistry.get(p.nodeId);
    if (!node?.proxy || node.connId !== client?.connId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "proxy node not owned by this connection"),
      );
      return;
    }
  }
