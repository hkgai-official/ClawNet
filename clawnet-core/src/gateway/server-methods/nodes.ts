...
import {
  ...
  validateNodeProxyRegisterParams,
  validateNodeProxyUnregisterParams,
...
export const nodeHandlers: GatewayRequestHandlers = {
  ...
  "node.list": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const deviceNodeEntries = deviceList.paired
          ...
          (entry) =>
            [
              entry.deviceId,
              {
                ...
                coreVersion: undefined as string | undefined,
                uiVersion: undefined as string | undefined,
                deviceFamily: undefined as string | undefined,
                modelIdentifier: undefined as string | undefined,
      ...
      const nodePairedEntries = nodeList.paired
        .filter((n) => !deviceNodeEntries.some(([id]) => id === n.nodeId))
        .map(
          (n) =>
            [
              n.nodeId,
              {
                ...
                fileAccess: n.fileAccess,
                tagFileAccess: n.tagFileAccess,
              },
            ] as const,
        );
      const pairedById = new Map([...deviceNodeEntries, ...nodePairedEntries]);
      ...
      const nodes = [...nodeIds].map((nodeId) => {
        ...
        return {
          ...
          fileAccess: live?.fileAccess ?? paired?.fileAccess,
          tagFileAccess: live?.tagFileAccess ?? paired?.tagFileAccess,
  ...
  "node.describe": async ({ params, respond, context }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const paired = deviceList.paired.find((n) => n.deviceId === id && isNodeEntry(n));
      const nodePaired = !paired ? nodeList.paired.find((n) => n.nodeId === id) : undefined;
      const connected = context.nodeRegistry.listConnected();
      const live = connected.find((n) => n.nodeId === id);

      if (!paired && !nodePaired && !live) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "unknown nodeId"));
        return;
      }

      const caps = uniqueSortedStrings([...(live?.caps ?? nodePaired?.caps ?? [])]);
      const commands = uniqueSortedStrings([...(live?.commands ?? nodePaired?.commands ?? [])]);
      ...
      respond(
        true,
        {
          ts: Date.now(),
          nodeId: id,
          displayName: live?.displayName ?? paired?.displayName ?? nodePaired?.displayName,
          platform: live?.platform ?? paired?.platform ?? nodePaired?.platform,
          version: live?.version ?? nodePaired?.version,
          coreVersion: live?.coreVersion ?? nodePaired?.coreVersion,
          uiVersion: live?.uiVersion ?? nodePaired?.uiVersion,
          deviceFamily: live?.deviceFamily ?? nodePaired?.deviceFamily,
          modelIdentifier: live?.modelIdentifier ?? nodePaired?.modelIdentifier,
          remoteIp: live?.remoteIp ?? paired?.remoteIp ?? nodePaired?.remoteIp,
          caps,
          commands,
          pathEnv: live?.pathEnv,
          permissions: live?.permissions ?? nodePaired?.permissions,
          connectedAtMs: live?.connectedAtMs,
          paired: Boolean(paired || nodePaired),
          connected: Boolean(live),
        },
        undefined,
      );
  ...
  "node.proxy.register": async ({ params, respond, context, client }) => {
    if (!validateNodeProxyRegisterParams(params)) {
      respondInvalidParams({
        respond,
        method: "node.proxy.register",
        validator: validateNodeProxyRegisterParams,
      });
      return;
    }
    const p = params as {
      ...
      platform?: string;
      deviceFamily?: string;
      fileAccess?: import("../../infra/node-pairing.js").NodeFileAccess;
    ...
    // Check if a node with this ID is already connected (non-proxy direct node).
    const existing = context.nodeRegistry.get(nodeId);
    if (existing && !existing.proxy) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "nodeId already connected as a direct node"),
      );
      return;
    }
    const cfg = loadConfig();
    ...
    const filtered = p.commands
      .map((cmd) => cmd.trim())
      .filter((cmd) => cmd.length > 0 && allowlist.has(cmd));
    // Capture stale proxy nodeIds BEFORE registerProxy (it will unregister them from memory)
    const existingProxies = context.nodeRegistry.proxyNodesByConn.get(connId);
    const staleNodeIds = existingProxies
      ? [...existingProxies].filter((nid) => nid !== nodeId)
      : [];

    const session = context.nodeRegistry.registerProxy(
      client as unknown as import("../server/ws-types.js").GatewayWsClient,
      {
        ...
        tagFileAccess: p.tagFileAccess,
      },
    );
    // Remove stale devices from paired.json
    for (const staleId of staleNodeIds) {
      unpairNode(staleId).catch((err) => {
        context.logGateway.warn(`unpairNode failed for stale ${staleId}: ${err}`);
      });
    }
    // Persist fileAccess + tagFileAccess to paired.json
    updatePairedNodeMetadata(nodeId, {
      fileAccess: p.fileAccess,
      tagFileAccess: p.tagFileAccess,
      lastConnectedAtMs: Date.now(),
    }).catch((err) => {
      context.logGateway.warn(`updatePairedNodeMetadata failed for ${nodeId}: ${err}`);
    });
    context.logGateway.info(
      `node.proxy.register nodeId=${nodeId} commands=${filtered.join(",")} conn=${connId}`,
    );
    respond(
      true,
      {
  ...
  "node.proxy.unregister": async ({ params, respond, context, client }) => {
    if (!validateNodeProxyUnregisterParams(params)) {
      respondInvalidParams({
        respond,
        method: "node.proxy.unregister",
        validator: validateNodeProxyUnregisterParams,
      });
      return;
    }
    const { nodeId } = params as { nodeId: string };
    ...
    const removed = context.nodeRegistry.unregisterProxy(nodeId, connId);
    if (!removed) {
      respond(
        false,
        undefined,
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          "proxy node not found or not owned by this connection",
        ),
      );
      return;
    }
    context.logGateway.info(`node.proxy.unregister nodeId=${nodeId} conn=${connId}`);
    respond(true, { nodeId, ok: true }, undefined);
  },
