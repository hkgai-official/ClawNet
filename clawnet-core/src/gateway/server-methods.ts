...
import { tagContextHandlers } from "./server-methods/tag-context.js";
...
function authorizeGatewayMethod(method: string, client: GatewayRequestOptions["client"]) {
  ...
  // Node-role methods (node.invoke.result, node.event, skills.bins) don't need operator scope checks.
  if (role === "node" || (role === "unified" && isNodeRoleMethod(method))) {
    return null;
  }
