...
export function attachGatewayWsConnectionHandler(params: {
  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    socket.once("close", (code, reason) => {
      ...
      if (client?.connect?.role === "node" || client?.connect?.role === "unified") {
      ...
      if (client?.connect?.role === "operator") {
        // Clean up any proxy nodes registered by this operator connection.
        const context = buildRequestContext();
        const removed = context.nodeRegistry.unregisterAllProxies(connId);
        if (removed.length > 0) {
          logGateway.info(
            `operator disconnect: removed ${removed.length} proxy node(s): ${removed.join(",")} conn=${connId}`,
          );
        }
      }
      logWs("out", "close", {
