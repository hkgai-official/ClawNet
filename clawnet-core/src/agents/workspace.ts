...
/**
 * Resolve the workspace directory for a tag-specific workspace.
 * Tag workspaces live under `~/.openclaw/workspace/{tagWorkspaceId}/`.
 * Used by the server to isolate agent persona/memory per social identity (tag).
 */
export function resolveTagWorkspaceDir(
  tagWorkspaceId: string,
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  // All tags (including default) live under workspace/{slug}/
  return path.join(
...
const KNOWN_WORKSPACE_ENTRIES = new Set([
  "SOUL.md", "AGENTS.md", "IDENTITY.md", "USER.md", "TOOLS.md",
  "HEARTBEAT.md", "MEMORY.md", "BOOTSTRAP.md",
  "memory", "skills", "Task",
  ".git", ".openclaw", ".clawhub", "node_modules",
]);

let _defaultMigrated = false;

export async function migrateDefaultWorkspaceIfNeeded(
  workspaceRoot: string,
): Promise<void> {
  if (_defaultMigrated) return;
  _defaultMigrated = true;

  const defaultDir = path.join(workspaceRoot, "default");
  const soulInRoot = path.join(workspaceRoot, "SOUL.md");

  // If default/ already exists or root has no bootstrap files, skip
  if (await fileExists(defaultDir) || !(await fileExists(soulInRoot))) {
    return;
  }

  // Move only known workspace entries into default/
  await fs.mkdir(defaultDir, { recursive: true });
  await inheritOwnership(defaultDir, workspaceRoot);
  const entries = await fs.readdir(workspaceRoot, { withFileTypes: true });
  for (const entry of entries) {
    if (!KNOWN_WORKSPACE_ENTRIES.has(entry.name)) continue;
    await fs.rename(
...
/**
 * Inherit ownership from a parent directory onto a target path.
 * When the gateway runs as root but workspace dirs belong to a non-root user
 * (e.g. UID 1001), newly created files/dirs would be owned by root.
 * This helper fixes that by chown-ing to match the parent.
 * No-op when running as non-root or when ownership already matches.
 */
async function inheritOwnership(targetPath: string, referenceDir?: string): Promise<void> {
  if (process.getuid?.() !== 0) return; // not root, nothing to do
  try {
    const ref = referenceDir ?? path.dirname(targetPath);
    const parentStat = await fs.stat(ref);
    if (parentStat.uid === 0) return; // parent is also root, skip
    const targetStat = await fs.stat(targetPath);
    if (targetStat.uid === parentStat.uid && targetStat.gid === parentStat.gid) return;
    await fs.chown(targetPath, parentStat.uid, parentStat.gid);
  } catch {
    // best-effort; don't break workspace creation
  }
}

/**
 * Recursively chown all files and directories under `dir` to match
 * the ownership of `referenceDir`.
 */
async function inheritOwnershipRecursive(dir: string, referenceDir: string): Promise<void> {
  if (process.getuid?.() !== 0) return;
  try {
    const parentStat = await fs.stat(referenceDir);
    if (parentStat.uid === 0) return;

    const chownRecursive = async (p: string) => {
      const st = await fs.stat(p);
      if (st.uid !== parentStat.uid || st.gid !== parentStat.gid) {
        await fs.chown(p, parentStat.uid, parentStat.gid);
      }
      if (st.isDirectory()) {
        const entries = await fs.readdir(p, { withFileTypes: true });
        for (const entry of entries) {
          await chownRecursive(path.join(p, entry.name));
        }
      }
    };
...
async function writeFileIfMissing(filePath: string, content: string): Promise<boolean> {
  try {
    ...
    await inheritOwnership(filePath);
    return true;
  } catch (err) {
...
async function ensureGitRepo(dir: string, isBrandNewWorkspace: boolean) {
  ...
  try {
    ...
    // Fix ownership of .git dir created by git init
    const gitDir = path.join(dir, ".git");
    await inheritOwnershipRecursive(gitDir, dir);
  } catch {
...
export async function ensureAgentWorkspace(params?: {
  ...
  await inheritOwnership(dir, parentDir);
