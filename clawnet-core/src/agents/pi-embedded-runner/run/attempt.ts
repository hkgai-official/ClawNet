...
import { filterContext } from "../../../embedding-filter/index.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    // Resolve tag context early — needed for both tool creation and system prompt
    const attemptTagCtx = params.sessionKey ? getSessionTagContext(params.sessionKey) : undefined;
    ...
    const appendPrompt = buildEmbeddedSystemPrompt({
      ...
      messageToolHints,
      sandboxInfo,
      tagWorkspaceId: attemptTagCtx?.workspaceId,
      a2aMode: attemptTagCtx?.a2aMode,
      tools,
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // --- Embedding filter: filter context by similarity before LLM call ---
          const embFilterResult = await filterContext({
            userPrompt: effectivePrompt,
            messages:
              activeSession.messages as unknown as import("../../../embedding-filter/chunker.js").AnyMessage[],
            systemPromptText: systemPromptText ?? "",
          });
          if (embFilterResult) {
            activeSession.agent.replaceMessages(
              embFilterResult.filteredMessages as unknown as AgentMessage[],
            );
            applySystemPromptOverrideToSession(activeSession, embFilterResult.filteredSystemPrompt);
            log.info(
              `[embedding-filter] messages: kept ${embFilterResult.stats.keptMessageChunks}/${embFilterResult.stats.totalMessageChunks} chunks, ` +
                `system: kept ${embFilterResult.stats.keptSystemSections}/${embFilterResult.stats.totalSystemSections} sections`,
            );
          }
          // --- End embedding filter ---
