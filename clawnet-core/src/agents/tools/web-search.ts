...
const SEARCH_PROVIDERS = ["brave", "perplexity", "grok", "gemini", "kimi", "serpapi"] as const;
...
function resolveSerpApiKey(search?: WebSearchConfig): string | undefined {
  const serpapi =
    search && typeof search === "object" && "serpapi" in search
      ? (search as Record<string, unknown>).serpapi
      : undefined;
  const fromConfig =
    serpapi && typeof serpapi === "object" && "apiKey" in (serpapi as Record<string, unknown>)
      ? normalizeSecretInput((serpapi as Record<string, string>).apiKey)
      : "";
  const fromEnv = normalizeSecretInput(process.env.SERPAPI_API_KEY);
  return fromConfig || fromEnv || undefined;
}
...
function resolveSearchProvider(search?: WebSearchConfig): (typeof SEARCH_PROVIDERS)[number] {
  ...
  if (raw === "") {
    // 0. SerpAPI
    if (resolveSerpApiKey(search)) {
      logVerbose(
        'web_search: no provider configured, auto-detected "serpapi" from available API keys',
      );
      return "serpapi";
    }
    // 1. Brave
...
async function runSerpApiSearch(params: {
  ...
  if (!res.ok) {
    await throwWebSearchApiError(res, "SerpAPI");
  }

  const data = (await res.json()) as Record<string, unknown>;
  const organicResults = Array.isArray(data.organic_results) ? data.organic_results : [];

  return organicResults.map((r: Record<string, unknown>) => ({
  ...
  if (params.provider === "serpapi") {
    ...
    const payload = {
      ...
      results: results.map((r) => ({
        ...
        url: r.url,
        snippet: wrapWebContent(r.snippet),
      })),
...
export function createWebSearchTool(options?: {
  ...
  const description =
    provider === "serpapi"
      ? "Search the web using SerpAPI (Google Search). Returns titles, URLs, and snippets from Google search results."
      : provider === "perplexity"
