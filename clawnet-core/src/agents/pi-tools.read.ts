...
export function wrapToolWorkspaceRootGuard(
  tool: AnyAgentTool,
  root: string,
  guardOptions?: { sessionKey?: string; operation?: "read" | "write" },
): AnyAgentTool {
  return wrapToolWorkspaceRootGuardWithOptions(tool, root, {
    sessionKey: guardOptions?.sessionKey,
    operation: guardOptions?.operation,
  });
}
...
export function wrapToolWorkspaceRootGuardWithOptions(
  ...
  options?: {
    containerWorkdir?: string;
    sessionKey?: string;
    /** "read" (default) or "write" — write operations are blocked in ro/a2a mode. */
  ...
  return {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      // Enforce accessMode/a2aMode for write operations
      if (options?.operation === "write" && options?.sessionKey) {
        const writeTagCtx = getSessionTagContext(options.sessionKey);
        if (writeTagCtx) {
          const effectiveMode = writeTagCtx.accessMode ?? "rw";
          if (effectiveMode === "ro") {
            const reason = "read-only mode: write operations are denied";
            reportBoundaryViolation({
              type: "a2a_write_denied",
              sessionKey: options.sessionKey,
              tagName: writeTagCtx.tagName,
              tagWorkspaceId: writeTagCtx.workspaceId,
              boundary: root,
              attemptedPath: String((args as Record<string, unknown>)?.file_path ?? ""),
              detail: reason,
            });
            throw new Error(reason);
          }
          if (writeTagCtx.a2aMode) {
            const reason = "A2A mode: write operations are denied for agent-to-agent sessions";
            reportBoundaryViolation({
              type: "a2a_write_denied",
              sessionKey: options.sessionKey,
              tagName: writeTagCtx.tagName,
      ...
      if (typeof filePath === "string" && filePath.trim()) {
        ...
        try {
          ...
          const fsTagCtx = options?.sessionKey
            ? getSessionTagContext(options.sessionKey)
            : undefined;
          reportBoundaryViolation({
            type: "file_workspace_escape",
            sessionKey: options?.sessionKey,
            tagName: fsTagCtx?.tagName,
            tagWorkspaceId: fsTagCtx?.workspaceId,
            boundary: root,
