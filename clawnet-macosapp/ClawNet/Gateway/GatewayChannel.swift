...
struct WebSocketTaskBox: @unchecked Sendable {
    ...
    init(task: any WebSocketTasking) {
        self.task = task
    }
...
struct WebSocketSessionBox: @unchecked Sendable {
    ...
    init(session: any WebSocketSessioning) {
...
enum GatewayAuthSource: String, Sendable {
...
actor GatewayChannelActor {
    ...
    func authSource() -> GatewayAuthSource { self.lastAuthSource }
    ...
    private func watchdogLoop() async {
        while self.shouldReconnect {
            guard await self.sleepUnlessCancelled(nanoseconds: 30 * 1_000_000_000) else { return }
    ...
    private func sendConnect() async throws {
        ...
        var client: [String: AnyCodable] = [
            ...
            "displayName": AnyCodable(clientDisplayName),
            "version": AnyCodable(
        ...
        client["deviceFamily"] = AnyCodable(InstanceIdentity.deviceFamily)
        if let model = InstanceIdentity.modelIdentifier {
            client["modelIdentifier"] = AnyCodable(model)
        }
        var params: [String: AnyCodable] = [
            ...
            "caps": AnyCodable(options.caps),
            "locale": AnyCodable(primaryLocale),
        ...
        if !options.commands.isEmpty {
            params["commands"] = AnyCodable(options.commands)
        }
        if !options.permissions.isEmpty {
            params["permissions"] = AnyCodable(options.permissions)
        }
        ...
        if let authToken {
            params["auth"] = AnyCodable(["token": AnyCodable(authToken)])
    ...
    private func handleConnectResponse(
        ...
        if let auth = ok.auth,
           let deviceToken = auth["deviceToken"]?.value as? String {
            ...
            let scopes = (auth["scopes"]?.value as? [AnyCodable])?
    ...
    private func watchTicks() async {
        // Use 3x tolerance instead of 2x to handle brief sleep/wake cycles gracefully
        let tolerance = self.tickIntervalMs * 3
        while self.connected {
            ...
            if let last = self.lastTick {
                ...
                if delta > tolerance {
                    // Probe with a ping before declaring dead — the connection
                    // may still be alive after a short sleep/network blip
                    if let task = self.task {
                        do {
                            try await task.sendPing()
                            // Ping succeeded — connection is alive, reset tick timer
                            self.lastTick = Date()
                            self.logger.info("gateway tick missed but ping succeeded; resetting")
                            continue
                        } catch {
                            // Ping also failed — connection is truly dead
                        }
                    }
                    self.logger.error("gateway tick missed and ping failed; reconnecting")
                    self.connected = false
    ...
    func send(method: String, params: [String: AnyCodable]?) async throws {
        try await self.connectOrThrow(context: "gateway connect")
        ...
        kind: String) throws -> (id: String, data: Data)
    {
        ...
        let paramsObject: AnyCodable? = params.map { entries in
            let dict = entries.reduce(into: [String: AnyCodable]()) { dict, entry in
                dict[entry.key] = AnyCodable(entry.value.value)
            }
